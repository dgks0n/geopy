...
def auto_async_side_effect(is_async):
    def auto_async_side_effect(side_effect=None):
        if not is_async:
            return side_effect
...
def mock_sleep(auto_async_side_effect, rate_limiter_cls):
    with patch.object(rate_limiter_cls, '_sleep') as mock_sleep:
        mock_sleep.side_effect = auto_async_side_effect()
        yield mock_sleep
...
async def test_min_delay(
    ...
    mock_func = MagicMock()
    mock_func.side_effect = auto_async_side_effect()
    min_delay = 3.5
...
async def test_sync_raises_for_awaitable():
    ...
    # silence `coroutine 'g' was never awaited`:
